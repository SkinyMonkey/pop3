// landscape_cpu.wgsl — CPU palette index variant.
// Replaces landscape.vert + landscape_cpu.frag.
...
// Group 1: Storage buffers and texture
...
fn calc_tex_coord(v: f32, shift: i32) -> i32 {
    return (i32(v / 8.0 * 4096.0) + shift * 32) % 4096;
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let coords_int = vec2<i32>(
        calc_tex_coord(in.coord3d_out.x, params.level_shift.x),
        calc_tex_coord(in.coord3d_out.y, params.level_shift.y),
    );
