// landscape_full.wgsl — Full CPU texture variant.
// Replaces landscape.vert + landscape_full.frag.
...
fn calc_tex_coord_f(v: f32, shift: i32) -> f32 {
    return v + f32(shift) * (1.0 / 128.0);
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let coordf = vec2<f32>(
        calc_tex_coord_f(in.coord3d_out.x / 8.0, params.level_shift.x),
    ...
    return vec4<f32>(c.rgb * shadow_factor * in.viewport_fade, c.a);
}
