            ...
            if first_elem < vele.len() {
                ...
                while vele_idx < vele.len() && !vele_visited.contains(&vele_idx) {
                    ...
                    let should_render = match layer_type {
                        ...
                        _ => element_tribe as u8 == unit_id,          // Unit features
                    };
...
struct CurrentAnimation {
    ...
    selected_unit: u8,   // 0-3: unit feature overlay
}
...
fn load_current_animation(
    ...
    println!("Parsing animation {} (tribe {}, unit {})...", current.index, current.selected_tribe, current.selected_unit);
    ...
    if let Some(ref anim) = parsed {
        ...
        let label_str = if is_static {
            format!("Anim {} | Tribe {} ({}) | Unit {} | STATIC POSE",
                    current.index, current.selected_tribe, tribe_name, current.selected_unit)
        } else {
            format!("Anim {} | Tribe {} ({}) | Unit {} | {} frames | {} elements",
                    current.index, current.selected_tribe, tribe_name, current.selected_unit,
...
fn handle_input(
    ...
    // U key: cycle unit features (0 → 1 → 2 → 3 → 0)
    if keyboard.just_pressed(KeyCode::KeyU) {
        let new_unit = (current.selected_unit + 1) % 4;
        current.selected_unit = new_unit;
        current.parsed = None;
        current.frame_materials.clear();
        println!("Switched to unit {}", new_unit);
