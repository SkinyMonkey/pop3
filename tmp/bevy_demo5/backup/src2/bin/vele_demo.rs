            ...
            if first_elem < vele.len() {
                ...
                while vele_idx < vele.len() && !vele_visited.contains(&vele_idx) {
                    ...
                    let should_render = match layer_type {
                        0 => true,  // Base layer: always render
                        1 => element_tribe as u8 == tribe_id,  // Type 1: tribe match
                        _ => element_tribe as u8 == tribe_id,   // Type 2+: tribe match
                    };
...
fn load_current_animation(
    ...
    println!("Parsing animation {} (tribe {})...", current.index, current.selected_tribe);
    ...
    if let Some(ref anim) = parsed {
        ...
        let label_str = if is_static {
            format!("Anim {} | {} | STATIC POSE", current.index, tribe_name)
        } else {
            format!("Anim {} | {} | {} frames | {} elements", current.index, tribe_name, frame_count, elem_count)
        };
...
fn handle_input(
    ...
    // T key: cycle tribe (0 → 1 → 2 → 3 → 0)
    if keyboard.just_pressed(KeyCode::KeyT) {
        let new_tribe = (current.selected_tribe + 1) % 4;
        current.selected_tribe = new_tribe;
        current.parsed = None;  // Trigger reload with new tribe filter
        current.frame_materials.clear();
